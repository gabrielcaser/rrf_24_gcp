...
## I'm not sure this does! If you know please tell me :)
hh_data <- hh_data %>%
    zap_formats() %>%
    zap_labels()
...
summary_table <- datasummary(
    hh_size + n_child_5 + n_elder ~ as.factor(district) * (Mean + SD), # I only picked up a few 
    data = hh_data,
...
balance_table <- datasummary_balance(
    sumvars ~ treatment, # I'm not sure what 'sumvars' references to
    data = balance_data,
    stars = TRUE,
...
stargazer(
    ...
    type = "text",
    out = file.path("Outputs","regression_table.txt")
)


## I'm not doing the graphs now (But I'm not lazy!!)
